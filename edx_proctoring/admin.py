...
class ProctoredExamSoftwareSecureReviewAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, review, form, change):
        ...
        provider_name = get_provider_name_by_course_id(
            review.exam['course_id']
        )
        get_backend_provider(provider_name).on_review_saved(
            review, allow_status_update_on_fail=True
        )

    def get_form(self, request, obj=None, **kwargs):
        form = super(ProctoredExamSoftwareSecureReviewAdmin, self).get_form(
            request, obj, **kwargs
        )
